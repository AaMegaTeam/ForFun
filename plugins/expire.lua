local function pre_process(msg)
  local data = load_data(_config.moderation.data)
       group_owner = data[tostring(msg.to.id)]['set_owner']
      if not group_owner then
        group_owner = "Not Setted"
      end
       group_link = data[tostring(msg.to.id)]['settings']['set_link']
      if not group_link then
        group_link = "Not Founded"
      end
...
function run(msg, matches)
	...
	if matches[1]:lower() == 'unexpire' then
		if not is_sudo(msg) then return end
		redis:hdel('expiretime',get_receiver(msg))
		return "تاریخ انقضای گروه:\nبه Unlimited تنظیم شد."
	end
